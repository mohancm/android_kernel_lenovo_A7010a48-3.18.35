				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	for (i = 0; i < ARRAY_SIZE(st_table); i++) {
		...
		if ((st_table[i].m == val2[0]) && (st_table[i].n == val2[1]))
			ucontrol->value.enumerated.item[1] = i;
	}
	return 0;
}
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	val2 = ucontrol->value.enumerated.item[1];
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.enumerated.item[0] = (max - val) & mask;
	ucontrol->value.enumerated.item[1] = (max - val2) & mask;
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	val_mask = mask << shift;
	val = ((max - ucontrol->value.enumerated.item[0]) & mask);
	val2 = ((max - ucontrol->value.enumerated.item[1]) & mask);
