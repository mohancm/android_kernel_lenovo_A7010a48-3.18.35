		    ...
		    struct snd_ctl_elem_value *ucontrol)
{
	...
	mutex_lock_nested(&codec->component.card->dapm_mutex, SND_SOC_DAPM_CLASS_RUNTIME);
	florida_set_dre(arizona, lshift, !!ucontrol->value.enumerated.item[0]);
	florida_set_dre(arizona, rshift, !!ucontrol->value.enumerated.item[1]);
	mutex_unlock(&codec->component.card->dapm_mutex);
	return 0;
}
			  ...
			  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.enumerated.item[0])
		val_l = mask_l;
	if (ucontrol->value.enumerated.item[1])
		val_r = mask_r;
	mutex_lock_nested(&codec->component.card->dapm_mutex, SND_SOC_DAPM_CLASS_RUNTIME);
