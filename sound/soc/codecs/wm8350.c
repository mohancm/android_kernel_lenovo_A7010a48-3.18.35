				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (out) {
		out->left_vol = ucontrol->value.enumerated.item[0];
		out->right_vol = ucontrol->value.enumerated.item[1];
		if (!out->active)
			return 1;
	}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	switch (reg) {
	case WM8350_LOUT1_VOLUME:
		ucontrol->value.enumerated.item[0] = out1->left_vol;
		ucontrol->value.enumerated.item[1] = out1->right_vol;
		return 0;

	case WM8350_LOUT2_VOLUME:
		ucontrol->value.enumerated.item[0] = out2->left_vol;
		ucontrol->value.enumerated.item[1] = out2->right_vol;
		return 0;
