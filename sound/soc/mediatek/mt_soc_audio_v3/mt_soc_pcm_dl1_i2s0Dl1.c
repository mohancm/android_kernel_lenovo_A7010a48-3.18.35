				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.enumerated.item[0] = mI2S0dl1_hdoutput_control;
	return 0;
}
...
static int Audio_I2S0dl1_hdoutput_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    ...
    printk("control=%d,value=%d\n",mI2S0dl1_hdoutput_control, ucontrol->value.enumerated.item[0]);
...
{
	AudDrv_Clk_On();
	ucontrol->value.enumerated.item[0] = Afe_Get_Reg(AFE_IRQ_MCU_CNT1);
	AudDrv_Clk_Off();
	return 0;
}
...
{
	irq1_cnt = ucontrol->value.enumerated.item[0];
