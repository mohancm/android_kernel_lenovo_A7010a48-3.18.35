...
{
	sIndia = ucontrol->value.enumerated.item[0];
...
static int india_region_headphone_get(struct snd_kcontrol *kcontrol,
                       struct snd_ctl_elem_value *ucontrol)
{
        ucontrol->value.enumerated.item[0] = sIndia;
        return 0;
}
// wangwy6 add for audio para of india end
...
static int florida_set_spk(struct snd_kcontrol *kcontrol,
		       struct snd_ctl_elem_value *ucontrol)
{
    if (gpio_get_value_cansleep(GPIO_ARIZONA_EXT_SPKEN_PIN) == ucontrol->value.enumerated.item[0])
    {
        printk("%s spk vdd already %s \n",__func__,ucontrol->value.enumerated.item[0] ? "On":"Off");
        return 0;
    }
    printk("%s: set spk vdd %s\n", __func__,ucontrol->value.enumerated.item[0] ? "On":"Off");
	gpio_set_value_cansleep(GPIO_ARIZONA_EXT_SPKEN_PIN, ucontrol->value.enumerated.item[0]);
	return 1;
}
...
static int florida_get_spk(struct snd_kcontrol *kcontrol,
		       struct snd_ctl_elem_value *ucontrol)
{
        ucontrol->value.enumerated.item[0] = gpio_get_value_cansleep(GPIO_ARIZONA_EXT_SPKEN_PIN);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (snd_soc_volsw_is_stereo(mc)) {
		...
		if (invert)
			ucontrol->value.enumerated.item[1] =
				max - ucontrol->value.enumerated.item[1];
	}
	return 0;
}
...
static int mt6735_get_lowjitter(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.enumerated.item[0] = mt_soc_lowjitter_control;
	return 0;
}
...
static int mt6735_set_lowjitter(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	mt_soc_lowjitter_control = ucontrol->value.enumerated.item[0];
	return 0;
}
